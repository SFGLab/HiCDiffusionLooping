...
def main(run):
    ...
    training_args = TrainingArguments(
        ...
        warmup_steps=0, # no need to warm up 
        num_train_epochs=10,
        ...
        save_total_limit=3,
        save_strategy='steps',
        save_steps=10 if DEBUG else 10_000,
        save_safetensors=False, # doesnt work with shared hicdiffusion modules
        logging_strategy='steps',
        logging_steps=10,
        # auto_find_batch_size=True,
        per_device_eval_batch_size=1 if DEBUG else 4,
        per_device_train_batch_size=1 if DEBUG else 4,
        run_name=RUN_NAME,
        disable_tqdm=True,
